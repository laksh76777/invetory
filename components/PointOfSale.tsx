      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8 h-full">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200/80 dark:border-slate-700/50 p-4 md:p-6 flex flex-col">
          <div className="w-full max-w-xl mx-auto text-center mb-6">
            ...
            <p className="text-slate-500 dark:text-slate-400 mb-4">{t('pos.main_instruction')}</p>
            ...
            {scanError && <p className="text-red-500 mt-3 font-semibold">{scanError}</p>}
          </div>
          {/* We can add a product grid here for quick adds in a future update */}
        </div>


        {/* Cart */}
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200/80 dark:border-slate-700/50 flex flex-col h-[calc(100vh-10rem)]">
          ...
          <div className="flex-1 p-2 overflow-y-auto">
              {cart.length === 0 ? (
                  ...
                  <ul className="space-y-2 p-2">
                      {cart.map(item => (
                          <li key={item.productId} className={`flex items-center justify-between p-3 rounded-lg transition-all ${item.productId === lastAddedProductId ? 'animate-highlight' : ''} ${item.productId === quantityErrorId ? 'animate-shake' : ''} hover:bg-slate-50 dark:hover:bg-slate-700/50`}>
                              ...
                              <div className="flex items-center gap-1 sm:gap-2">
                                  ...
                                  <span className={`w-8 text-center font-semibold ${item.productId === quantityErrorId ? 'text-red-500' : 'text-slate-800 dark:text-slate-100'}`}>{item.quantity}</span>
                                  <button onClick={() => updateQuantity(item.productId, item.quantity + 1)} className="h-8 w-8 flex items-center justify-center border border-slate-300 dark:border-slate-600 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 transition">+</button>
